...
async def run_test_class(test_class_name: str, client: DummyClient, specific_method: str | None = None):
    """
    ...
    try:
        ...
        if not test_class:
            if hasattr(module, "run") and not specific_method:
                ...
                log.error(f"Cannot run specific method '{specific_method}' on legacy test module {test_class_name}")
                return False
            else:
        ...
        if specific_method:
            ...
            methods = [
                func for func in dir(test_class) if callable(getattr(test_class, func)) and func.startswith("test_")
            ]
        ...
        for method_name in methods:
            ...
            try:
                ...
                asyncio.create_task(client.send_to_channel(f"🟢 {method_name} PASSED"))
            except AssertionError as e:
                log.error(f"FAIL: {method_name} - Assertion Error: {e}")
                asyncio.create_task(client.send_to_channel(f"🔴 {method_name} FAILED\n**Reason:** {e}"))
                all_passed = False
...
async def run_all_tests(client: DummyClient):
    ...
    log.info("-" * 30)
    log.info(f"Total: {len(results)}, Passed: {passed}, Failed: {len(results) - passed}")
...
async def main():
    ...
    send_parser.add_argument("--no-wait", action="store_true", help="Don't wait for a response")
    run_parser = subparsers.add_parser("run", help="Run test cases")
    run_parser.add_argument("test_name", help="Name of the test case (filename without extension) or 'all'")
    run_parser.add_argument("method_name", nargs="?", help="Specific method to run (optional)")
