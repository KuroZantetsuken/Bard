...
class ScraperCacheTest(BardTestCase):
    async def test_scraper_cache_hit(self):
        ...
        pop1 = await self.bot.send_and_wait(f"<@{self.bot.settings.BOT_ID}> Summarize this page: {url}")
        ...
        start_time = time.time()
        hit1 = await self.bot.send_and_wait(f"<@{self.bot.settings.BOT_ID}> Read this link again: {url}")
        ...
        start_time = time.time()
        pop2 = await self.bot.send_and_wait(f"<@{self.bot.settings.BOT_ID}> Check this video: {url_video}")
        ...
        start_time = time.time()
        hit2 = await self.bot.send_and_wait(f"<@{self.bot.settings.BOT_ID}> Check this video again: {url_video}")
        ...
        self.assertTrue(duration_warm < duration_cold, duration_video_warm < duration_video_cold)
