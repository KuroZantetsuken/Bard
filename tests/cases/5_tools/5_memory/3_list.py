...
class MemoryReadTest(BardTestCase):
    async def test_memory_read(self):
        ...
        """
        check1 = await self.bot.send_and_wait(f"<@{self.bot.settings.BOT_ID}> What do you know about me?")
        self.assertIn("1234", check1.content)
        print(f"Response: {check1.content}")
        response = await self.bot.send_and_wait(f"<@{self.bot.settings.BOT_ID}> What do you remember about me?")
