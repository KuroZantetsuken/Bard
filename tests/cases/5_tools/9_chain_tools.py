...
class ToolsChainingTest(BardTestCase):
    async def test_tools_chaining(self):
        ...
        self.assertTrue(
            len(response.attachments) >= 3, f"Expected at least 3 attachments, found {len(response.attachments)}"
        )
        content = response.content.lower()
        self.assertTrue("remember" in content or "memory" in content or "support" in content or "note" in content)
        events = await guild.fetch_scheduled_events()
