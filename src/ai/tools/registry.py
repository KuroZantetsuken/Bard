...
from ai.tools.base import AttachmentProcessorProtocol, BaseTool, GeminiCoreProtocol, ToolContext
...
class ToolRegistry:
    ...
    def _discover_and_load_tools(self):
        ...
        """
        tools_dir = "src/ai/tools"
        tools_dir_name = "src.ai.tools"
        tools_abs_path = os.path.abspath(tools_dir)
        if not os.path.isdir(tools_abs_path):
            log.warning(f"Tools directory '{tools_abs_path}' not found. No tools will be loaded.")
            return
        ...
        for filename in os.listdir(tools_abs_path):
            if filename.endswith(".py") and not filename.startswith("_") and filename != "__init__.py":
                ...
                try:
                    ...
                    if spec is None:
                        log.error(f"Could not create module spec/module for '{logical_module_name}'. Skipping.")
                        continue
                    ...
                    for attribute_name in dir(module):
                        ...
                        if inspect.isclass(attribute) and issubclass(attribute, BaseTool) and attribute is not BaseTool:
                            try:
                                tool_instance = attribute(context=self.shared_tool_context)
                                if hasattr(tool_instance, "tool_emoji") and tool_instance.tool_emoji:
                                    self.tool_emojis[attribute.__name__] = tool_instance.tool_emoji
                                ...
                                for func_decl in tool_instance.get_function_declarations():
                                    if func_decl.name is None:
                                        log.warning(f"Skipping function without name in {attribute.__name__}")
                                        continue
    ...
    def reset_tool_context_data(self):
        ...
        if self.shared_tool_context:
            ...
            log.warning("Attempted to reset tool context data, but shared_tool_context is None.")
    ...
    async def execute_function(
        ...
        """
        log.info(f"Executing tool function: {function_name}")
        ...
        if not tool_class_name:
            log.error(f"No registered tool found for function: {function_name}.")
        ...
        try:
            ...
            if (
                ...
                log.info(
                    f"Tool function {function_name} returned",
                    extra={"response": result.function_response.response},
                )
            else:
                log.info(f"Tool function {function_name} returned", extra={"result": result})
            ...
            log.error(
                f"Function '{function_name}' in tool '{tool_class_name}' timed out after {context.settings.TOOL_TIMEOUT_SECONDS} seconds."
            )
            return gemini_types.Part(
                function_response=gemini_types.FunctionResponse(
                    name=function_name,
