                ...
                for candidate in response.candidates:
                    for part in candidate.content.parts:
                        if part.inline_data and part.inline_data.mime_type.startswith(
                            ...
                            # Set the image data and filename in tool_response_data
                            ...
                            break  # Assuming only one image per generation call
                    if image_generated:
                        break
            else:
                feedback = response.prompt_feedback
                feedback_str = str(feedback) if feedback else "No candidates returned."
                logger.error(f"Image generation failed. Feedback: {feedback_str}.")
                return types.Part(
