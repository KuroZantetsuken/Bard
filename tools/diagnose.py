...
# Initialize logger for the diagnose tool module.
...
class DiagnoseTool(BaseTool):
    ...
    def _get_directory_hierarchy(self, path: str) -> str:
        ...
        for root, dirs, files in os.walk(path, topdown=True):
            # Exclude dunder directories (e.g., __pycache__) and dot directories (e.g., .git)
            dirs[:] = [
                d
                for d in dirs
                if not d.startswith(".")
            ...
            # Skip the root folder if it's a dot or dunder folder
            if level == 0 and (
