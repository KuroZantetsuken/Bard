...
# Initialize logger for the context module.
...
class ChatHistoryManager(JsonStorageManager):
    ...
    async def load_history(
        ...
        for entry in raw_history:
            try:
                # Deserialize timestamp, handling potential timezone information.
    ...
    def _filter_entries(self, entries: List[HistoryEntry]) -> List[HistoryEntry]:
        ...
        # The Gemini API expects contents to be role-paired (user, model, user, model...).
        # To maintain pairs, we keep `max_history_turns * 2` entries.
